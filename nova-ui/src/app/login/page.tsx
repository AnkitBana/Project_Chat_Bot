    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)" }}>
      <div style={{ width: "100%", maxWidth: 400, padding: "0 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: 14, marginTop: 4 }}>Sign in to Nova AI</p>
        </div>

        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {["username", "password"].map((field) => (
            <div key={field}>
              <label style={{ fontSize: 13, fontWeight: 500, display: "block", marginBottom: 6, textTransform: "capitalize" }}>{field}</label>
              <input
        ...
        <p style={{ textAlign: "center", marginTop: 20, fontSize: 13, color: "var(--muted)" }}>
          ...
          <a href="/signup" style={{ color: "var(--accent2)", textDecoration: "none", fontWeight: 500 }}>Sign up</a>
        </p>
      </div>
    </div>
  );
